...
def gestionar_envido(puntaje_equipo_1, puntaje_equipo_2):
    ...
    if se_jugo:
        ganador = input("¿Quién ganó el envido? (1/2): ")
        puntos = puntos_envido(opcion, puntaje_equipo_1, puntaje_equipo_2, ganador)
        if ganador == "1":
            print(f"Se anotarán {puntos} puntos al equipo {ganador}\n")
            ...
            return gestionar_envido(
                puntaje_equipo_1, puntaje_equipo_2
            )  # en caso de que cargue una opción no válida vuelve a preguntar
    else:
